...
export const WhiteLabel = () => {
  ...
  const {
    ...
    isLoading,
    hasError,
  } = useMemberManagement("whitelabel", currentUser) || {
  ...
  const handleError = useCallback(
    (error, context = "") => {
      ...
      console.error(`WhiteLabel component error ${context}:`, error);
      ...
      const initializeData = async () => {
        ...
        try {
          ...
          const promises = [
            ...
            // For WhiteLabel creation, fetch Admin parents created by current user
            fetchAvailableParents("whitelabel").catch((err) => {
          ...
          console.error("Failed to initialize whitelabel data:", error);
  ...
  const transformedData = useMemo(() => {
    ...
    const transformed = members.map((member) => ({
      ...
      type: member.role_name || member.role || "whitelabel",
      parentName: member.parent_name || "Admin",
      parentMobile: member.parent_user_code || "ADMIN",
      parentRole: "Admin",
  ...
  const handleDelete = useCallback(
    async (memberId) => {
      ...
      const confirmed = window.confirm(
        "Are you sure you want to delete this whitelabel member? This action cannot be undone."
      );
      if (!confirmed) return;
      ...
      return handleActionWithLoading(
        ...
        "WhiteLabel member deleted successfully",
        "delete member"
      );
    },
  ...
  const handleFormSubmit = useCallback((formData) => {
    console.log("WhiteLabel form submitted:", formData);
    setEditModal(null);
  }, []);

  const handleStockSubmit = useCallback((type, value) => {
    console.log(`WhiteLabel stock submitted [${type}]: ${value}`);
  }, []);
  ...
  const handleExport = useCallback(async () => {
    return handleActionWithLoading(
      async () => {
        ...
        a.href = url;
        a.download = `whitelabel_members_${
      ...
      "WhiteLabel members exported successfully",
      "export"
    );
  ...
  const handleRefresh = useCallback(async () => {
    return handleActionWithLoading(
      ...
      "WhiteLabel data refreshed successfully",
      "refresh"
    );
    ...
    <div className="h-[90vh] 2xl:max-w-[80%] p-4 mx-8 dark:bg-darkBlue/70 rounded-2xl 2xl:mx-auto text-gray-800 overflow-hidden overflow-y-auto px-4 pb-6 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
      <div className="my-4 p-4 rounded-md bg-white dark:bg-transparent">
        <div className="flex gap-3 justify-between">
          <h2 className="text-2xl font-bold dark:text-adminOffWhite">
            WhiteLabel List {totalMembers > 0 && `(${totalMembers})`}
          </h2>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-2">
          <Link to="create" className="btn-24 bg-blue-600">
            Create WhiteLabel
          </Link>
        </div>
      </div>
