...
export const MasterDistributor = () => {
  ...
  const {
    ...
    isLoading,
    hasError,
  } = useMemberManagement("mds", currentUser) || {
  ...
  const handleError = useCallback(
    (error, context = "") => {
      ...
      console.error(`MasterDistributor component error ${context}:`, error);
      ...
      const initializeData = async () => {
        ...
        try {
          ...
          const promises = [
            ...
            // For MDS creation, fetch WhiteLabel parents created by current user
            fetchAvailableParents("mds").catch((err) => {
          ...
          console.error("Failed to initialize mds data:", error);
  ...
  const transformedData = useMemo(() => {
    ...
    const transformed = members.map((member) => ({
      ...
      type: member.role_name || member.role || "mds",
      parentName: member.parent_name || "WhiteLabel",
      parentMobile: member.parent_user_code || "WHITELABEL",
  ...
  const handleDelete = useCallback(
    async (memberId) => {
      ...
      const confirmed = window.confirm(
        "Are you sure you want to delete this MDS member? This action cannot be undone."
      );
      if (!confirmed) return;
      ...
      return handleActionWithLoading(
        ...
        "MDS member deleted successfully",
        "delete member"
      );
    },
  ...
  const handleBulkStatusUpdate = useCallback(
    async (memberIds, newStatus) => {
      return handleActionWithLoading(
        ...
        `MDS members ${newStatus ? "activated" : "deactivated"} successfully`,
        "bulk status update"
      );
    },
  ...
  const handleFormSubmit = useCallback((formData) => {
    console.log("MDS form submitted:", formData);
    setEditModal(null);
  }, []);

  const handleStockSubmit = useCallback((type, value) => {
    console.log(`MDS stock submitted [${type}]: ${value}`);
  }, []);
  ...
  const handleExport = useCallback(async () => {
    return handleActionWithLoading(
      ...
      "MDS members exported successfully",
      "export"
    );
  ...
  const handleRefresh = useCallback(async () => {
    return handleActionWithLoading(
      ...
      "MDS data refreshed successfully",
      "refresh"
    );
