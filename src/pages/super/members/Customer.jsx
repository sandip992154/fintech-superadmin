...
export const Customer = () => {
  ...
  const {
    ...
    isLoading,
    hasError,
  } = useMemberManagement("customer", currentUser) || {
  ...
  const handleError = useCallback(
    (error, context = "") => {
      ...
      console.error(`Customer component error ${context}:`, error);
      ...
      const initializeData = async () => {
        ...
        try {
          ...
          const promises = [
            ...
            fetchAvailableParents("customer").catch((err) => {
  ...
  const transformedData = useMemo(() => {
    ...
    const transformed = members.map((member) => ({
      ...
      type: member.role_name || member.role || "customer",
      parentName: member.parent_name || "Retailer",
      parentMobile: member.parent_user_code || "RETAILER",
      parentRole: "Retailer",
  ...
  const handleEnhancedFormSubmit = useCallback(
    async (formData) => {
      ...
      return handleActionWithLoading(
        ...
        `Customer member ${action} successfully`,
        `form submit ${action}`
      );
    },
    [
  ...
  const handleDelete = useCallback(
    async (memberId) => {
      ...
      const confirmed = window.confirm(
        "Are you sure you want to delete this customer member? This action cannot be undone."
      );
      if (!confirmed) return;
      ...
      return handleActionWithLoading(
        ...
        "Customer member deleted successfully",
        "delete member"
      );
    },
  ...
  const handleStockSubmit = useCallback((type, value) => {
    console.log(`Customer stock submitted [${type}]: ${value}`);
  }, []);
  ...
  const handleExport = useCallback(async () => {
    return handleActionWithLoading(
      ...
      "Customer members exported successfully",
      "export"
    );
      ...
      <div className="my-4 p-4 rounded-md bg-white dark:bg-transparent">
        <div className="flex gap-3 justify-between">
          <h2 className="text-2xl font-bold dark:text-adminOffWhite">
            Customer List {totalMembers > 0 && `(${totalMembers})`}
          </h2>
